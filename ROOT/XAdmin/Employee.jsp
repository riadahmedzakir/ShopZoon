...
<div class="part2" id="adminTable" style="width: 100%;">
	<center>
		...
		<%	
			...
			ArrayList<People> pcList = (ArrayList<People>)obj.showUser("where active=1 and type!='Customar'");
			for(People pc: pcList){   
				out.println("<tr><td>"+pc.getFname()+ " "+pc.getLname()+"</td><td>"+pc.getType()+"</td><td>"+pc.getRating()+" %</td></td>");
				out.print("<td><input type='submit' value='View' style='color:Green;border:none;background:none;cursor:pointer' onclick='viewUser(\""+pc.getId()+","+pc.getFname()+","+pc.getLname()+","+pc.getEmail()+","+pc.getType()+","+pc.getRating()+","+pc.getActive()+","+pc.getLocation()+","+pc.getPhone()+"\")'></td></tr>");
			}   
		%> 
		</table>  
	</center>
...
function viewUser(val)
{
	...
	var rating="<tr><td>Rating</td><td>"+sp[5]+ " %</td><tr>";
	var active="<tr><td>Active</td><td>"+sp[6] + "</td><tr>";
	var address="<tr><td>Address</td><td>"+sp[7] + "</td><tr>";
	var phone="<tr><td>Phone</td><td>"+sp[8] + "</td><tr>";
	txt = ""+fname+lname+email+type+rating+active+address+phone+"";
...
{ 
	if (confirm('Want to '+val1+' This User ?') == true) {
